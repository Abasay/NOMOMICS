...
const ComicsSort = (props: {
  title: string;
  active: string;
  typeToSort: 'category' | 'genre' | 'location';
}) => {
  const { title, active, typeToSort } = props;
  const dummyArr = [1, 2, 3, 4, 5, 5, 6, 7, 8, 1, 2, 3, 4, 5, 6];
  const { comics } = useComics();

  const [comicsSorted, setComicsSorted] = React.useState(
    active === 'all'
      ? comics
      : comics.filter((comic) => comic[typeToSort] === active)
  );
    ...
    return (
      <div className='container mx-auto flex flex-col gap-8 pt-20'>
        ...
        <div className='grid place-content-center pb-6'>
          <div className='text-center text-xl font-meidum'>
            No comics found in this category
          </div>
        </div>
      </div>
    );
