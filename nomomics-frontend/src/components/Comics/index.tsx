...
const Comics = (props: { title: string }) => {
  ...
  return (
    <div className='container  dark:text-white mx-auto flex  flex-col gap-8 pt-10 '>
      ...
      <div className='grid grid-cols-[repeat(auto-fill,minmax(190px,1fr))] gap-10 w-[90%] h-full mx-auto max-md:w-[99%] max-md:flex scrollbar-hide max-md:gap-3 max-md:overflow-scroll max-md:scroll-smooth'>
        {comics.map(
          (
            _: {
              title: string;
            ...
            <>
              <Link
                href={'/details/' + _._id}
                className='relative cursor-pointer max-480:hidden min-w-[180px] max-[380px]:min-w-[50px] max-md:min-h-[250px] max-h-[250px] max-480:max-h-[250px] max-w-[200px] min-h-[250px]'
                key={index}
              >
                <Image
              ...
              <Link
                href={'/details'}
                className={` relative max-480:flex flex-col gap-0 cursor-pointer min-w-[200px] hidden border  min-h-[100px] max-h-[220px] max-w-[200px] rounded-lg overflow-hidden`}
                key={index}
              >
                <Image src={_.coverImage} alt={_.title} width={200} height={200} className='w-full h-full' />
      ...
      <div className=' w-[150px] mx-auto'>
        <Button1 text='See more' className=' w-full bg-secondary py-2' onClickFunc={() => console.log('usee more')} />
      </div>
    </div>
  );
};
