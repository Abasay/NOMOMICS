...
const ReadComic = () => {
  ...
  useEffect(() => {
    if (comic) {
      setNextEpisode(comic.episodes.findIndex((episode) => episode.episodeNumber === Number(episodeNumber)));
    }
  }, [comic]);

  useEffect(() => {
    if (comic) setNextEpisode(comic.episodes.findIndex((episode) => episode.episodeNumber === Number(episodeNumber)));
  }, [episodeNumber]);
        ...
        <div className='w-[90%] container mx-auto'>
          <div className='relative mb-10 flex py-16 flex-row gap-2 bg-[#FAE8E6] w-full px-14'>
            ...
            <div className='flex overflow-x-auto gap-4 scrollbar-hide'>
              {[...Array(9)].map((_, index) => (
                <div key={index} className='w-24 h-36 bg-gray-200 rounded animate-pulse'></div>
              ))}
            </div>
  ...
  if (!comic || !comic.episodes.find((e) => Number(e.episodeNumber) === Number(episodeNumber))) {
    Swal.fire({
      icon: 'error',
      title: 'Oops...',
        ...
        <div className='container mx-auto relative'>
          <div className=' w-full py-12 max-md:text-sm   flex flex-col gap-10 font-comic items-center justify-center'>
            <div className=' flex flex-col gap-5 max-480:gap-2.5 items-center justify-center'>
              ...
              <h4 className='text-2xl max-md:text-sm tracking-widest'>By: {comic?.author}</h4>
              <h5 className='text-xl max-md:text-sm tracking-widest font-semibold'>Episode {episodeNumber}</h5>
            </div>
          </div>
          {comic.episodes[nextEpisode - 1] ? (
            <Link href={`/read/${comic._id}?episode=${comic.episodes[nextEpisode - 1].episodeNumber}`}>
              <ButtonBack
        ...
        <div className=' relative mb-10 flex py-16 flex-row gap-2 bg-[#FAE8E6] w-full px-14'>
          ...
          >
            {comic.episodes.map((episode, index) => (
              <Episodes key={index} episode={episode} comic={comic} />
            ))}
          </div>
