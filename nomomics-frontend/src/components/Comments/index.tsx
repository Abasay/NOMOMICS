...
const CommentComp = () => {
	...
	const [replyingComment, setReplyingComment] = useState<Comment | null>(
		null
	);
	...
	const likeAndUnlikeAComment = async (comment: Comment) => {
		...
		try {
			const likeOrUnlike = await fetch(
				`${process.env.NEXT_PUBLIC_API_URL}/comics/like-and-unlike-a-comment`,
				{
					method: 'POST',
					headers: {
						'Content-Type':
