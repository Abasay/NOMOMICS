...
const CommentComp = () => {
  const { comments, setComments } = useComics();
  const {
    profile: { _id },
  } = useProfile();
  ...
  const likeAndUnlikeAComment = async (comment: Comment) => {
    const { comicId, userId, episodeId } = comment;

    if (!_id)
      return Swal.fire({
        icon: 'info',
        title: 'Login required!',
        text: 'Please log in to like this comment',
      });
    if (!comicId || !userId || !episodeId)
      Swal.fire({
        icon: 'error',
        title: 'An error occurred!',
        text: "You can't like this comment",
      });

    try {
      const likeOrUnlike = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/comics/like-and-unlike-a-comment`, {
        method: 'POST',
        headers: {
      ...
      const response = await likeOrUnlike.json();
      ...
      Swal.fire({
        icon: 'error',
        title: 'An error occurred!',
        text: 'An error occured liking this comment!',
      });
    }
  };

  const renderComment = (comment: Comment) => (
    <div key={comment._id} className='flex flex-col items-start mt-6 dark:text-gray-200'>
      <div className=' flex flex-row gap-4'>
        <Image
          src={comment.userId.profileImage || dummyImage}
        ...
        <div className='flex items-center space-x-2 text-[22px] leading-7 tracking-wider max-md:text-base max-480:text-sm '>
          <span className='font-bold'>{comment.userId.nickName || comment.userId.fullName.split(' ')[0]}</span>
      ...
      <div className='flex-1 flex-col'>
        <p className='mt-1 text-[#272727] text-xl tracking-wider  dark:text-gray-200'>
          <SlateRenderer content={JSON.parse(comment.content)} />
        </p>
        <div className='flex items-center text-[#8991A0] space-x-4 mt-2 text-sm'>
          ...
          <button className=' space-x-4 items-end' onClick={() => likeAndUnlikeAComment(comment)}>
            <svg
              width='24'
              height='24'
              viewBox='0 0 24 24'
              fill={comment.likes.find((like) => String(like.user) === String(_id)) ? '#f6b32a' : 'none'}
        ...
        {comment.replies && comment?.replies?.length > 0 && (
          <div className='ml-20 my-10'>{comment.replies.map((reply: any) => renderComment(reply))}</div>
        )}
      </div>
    </div>
  );
  return (
    <div className=' flex flex-col w-full gap-0 h-auto font-roboto dark:bg-inherit dark:text-gray-200'>
      ...
      <div className=' border rounded-md shadow-lg  dark:bg-slate-900 dark:text-gray-200'>
        <div className=' flex border-b-[1px] border-slate-100 rounded-md pb-10 pt-3 flex-row px-0 justify-between'>
          <h1 className=' px-4  text-2xl tracking-widest'>Comments</h1>
        </div>
            ...
            <div className='w-[90%] mx-auto mt-8 space-y-6'>
              <div className='w-full mx-auto mt-8 space-y-6'>
                {comments.length > 0 ? (
                  ...
                  <p className='text-lg max-md:text-sm text-center text-gray-500 max-480:text-xs'>
                    No comments yet. Be the first to comment.
                  </p>
                )}
              </div>
              <div className=' py-10 pt-0 w-full'>
                <CommentBox replyingComment={replyingComment} setReplyingComment={setReplyingComment} />
              </div>
              <div></div>
            </div>
          </div>
