...
import { AiOutlineBold, AiOutlineItalic, AiOutlineLink, AiOutlineFile, AiOutlinePicture } from 'react-icons/ai';
...
const Leaf = ({ attributes, children, leaf }: any) => {
  ...
  if (leaf.link) {
    children = (
      <a href={leaf.url} target='_blank' rel='noopener noreferrer' className='text-blue-500 underline'>
        {children}
      </a>
    );
  }
...
const CommentBox = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const postComment = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/comics/make-a-comment`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          comicId: comicId,
          episodeId: comic?._id,
          parentCommentId: replyingComment?._id,
        }),
      });
  ...
  return (
    <div
      className='w-full mx-auto my-3 mb-6 border border-gray-300 rounded-lg bg-white dark:bg-inherit dark:text-white'
      onClick={() => {
        // setShowLinkInput(false);
        // setShowImageInput(false);
      }}
    >
      {replyingComment && (
        <div key={replyingComment._id} className='flex pl-4 gap-4 max-md:flex-col relative items-start mt-6'>
          <div className=' absolute top-0 h-full w-full bg-white bg-opacity-45'></div>
          <div className=' flex flex-row gap-4'>
            <Image
              src={replyingComment.userId.profileImage || dummyImage}
              alt={replyingComment.userId.fullName}
              width={40}
              height={40}
              className='w-10 h-10 rounded-full'
            />
            <div className='flex items-center space-x-2 text-[22px] leading-7 tracking-wider '>
              <span className='font-bold'>{replyingComment.userId.nickName}</span>
            </div>
          </div>

          <div className='flex-1 flex-col'>
            <p className='mt-1 text-[#272727] text-xl tracking-wider'>
              <SlateRenderer content={JSON.parse(replyingComment.content)} />
            </p>
        ...
        >
          ...
          <div className='flex relative space-x-4 rounded-tr-md w-[150px] py-1 justify-between h-[46px] bg-gray-200 pl-4'>
            <div className=' flex gap-4'>
              <button onClick={handleBold} className='text-gray-700  text-xl hover:text-black'>
                <AiOutlineBold />
              </button>
              <button onClick={handleItalic} className='text-gray-700  text-xl hover:text-black'>
                <AiOutlineItalic />
              </button>
