...
const ConverterForm = () => {
  ...
  return (
    <div className='p-4 py-10 max-md:text-sm bg-white max-w-lg w-full mx-auto'>
      <div className='flex items-center justify-around mb-4'>
        <label className='flex items-center space-x-2'>
          <input
            ...
            name='conversionType'
            value='cashToToken'
            checked={conversionType === 'cashToToken'}
            onChange={() => setConversionType('cashToToken')}
            className='form-radio h-4 w-4 text-blue-600'
          />
          <span className=' text-[#606060]'>Cash to Token</span>
        </label>
        <label className='flex items-center space-x-2'>
          <input
            type='radio'
            name='conversionType'
            value='coinToToken'
            checked={conversionType === 'coinToToken'}
            onChange={() => setConversionType('coinToToken')}
            className='form-radio h-4 w-4 text-blue-600'
          />
          <span className=' text-[#606060]'>Coin to Token</span>
        </label>
      </div>
      ...
      <p className='text-sm text-gray-500 mb-2'>Conversion rate varies.</p>
      <p className='text-sm text-gray-500 mb-4'>1 Coin = 1 Token</p>
      ...
      <div className='bg-[#FDFDFD] border border-blue-300 flex gap-2 items-center p-4 rounded-md mb-4'>
        <span>
          ...
          >
            <g clip-path='url(#clip0_2450_4855)'>
              <path
        ...
        <div>
          ...
          <p className='text-sm text-gray-500 '>
            {coinAmount ? calculateTokens(Number(coinAmount)) : '0'} tokens
          </p>
        </div>
      </div>

      <div className=' bg-[#FDFDFD] mb-4'>
        <label htmlFor='coinAmount'>Enter your coin amount</label>
        <input
          type='number'
        ...
        <label htmlFor='tokenToreceive'>Token to recieve</label>
        <input
          type='number'
          placeholder='Token to receive'
          value={coinAmount ? calculateTokens(Number(coinAmount)) : ''}
          disabled
