...
const FileUpload = ({
	...
	const [showUploadDetails, setShowUploadDetails] =
	...
	const onDrop = async (acceptedFiles: File[]) => {
		...
		const convertedFiles = await Promise.all(
			acceptedFiles.map(async (file) => {
				const convertedFile = (await pdfToBase64(
					file
				)) as string;
	...
	useEffect(() => {
		const findComic = myComics.find(
			(comic) =>
				comic.title === title && comic.author === author
		);
