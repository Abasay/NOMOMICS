	...
	const getNotifications = async () => {
		try {
			...
			const request = await fetch(
				`${process.env.NEXT_PUBLIC_API_URL}/users/user/get-notifications`,
				{
					method: 'GET',
					headers: {
						'Content-Type':
			...
			if (response.success) {
				...
				setDraftCount(
					response.data.episodesUnApproved.length
				);
		...
		<div className=' mt-4 font-inter w-full container mx-auto'>
			...
			<div className=' flex max-md:flex-col gap-4 bg-[#ffffff]  border-teal-950'>
				...
				{active === 'Profile Settings' && (
					<div
						className={`w-full ${styles['fade-in']} overflow-auto`}
					>
						<ProfileSettings
							isCreator={isCreator}
						/>
