...
import { FaFacebook, FaGoogle, FaApple, FaEyeSlash, FaEye } from 'react-icons/fa';
  ...
  return (
    <div className='flex justify-center h-[80%]  items-center '>
      <div className='bg-white  p-8 pt-16 rounded-[25px] shadow-lg w-[80%] max-480:w-full px-16 min-w-md max-md:w-full max-md:px-8 '>
        <SignupSectionHeader title='Welcome back!' subTitle='Log in to your account' />
        <div className=' mt-6'>
          {errMsg && (
            <div className={`${errMsg ? styles['slide-in'] : styles['slide-out']}`}>
              {<Error message={errMsg} message1={errMsg1} type='login' w='full' />}
            </div>
          )}
          <form className=' pt-10 max-480:text-xs max-md:text-sm text-lg' onSubmit={handleSubmit}>
            <div className='mb-4'>
            ...
            <div className='mb-4 relative '>
              ...
              <input
                ...
                required
              />
              <span onClick={handleTogglePassword} className='absolute  right-4 top-1/2 text-zinc-500 cursor-pointer'>
                {showPassword ? <FaEye size={24} className='' /> : <FaEyeSlash size={24} />}
              </span>
            </div>
            <div className='mb-10 flex w-full flex-wrap  justify-between'>
              ...
              <span className=' text-zinc-500 max-md:text-xs text-[14px]'>Forgot password?</span>
            </div>
            <Button
              className='w-full'
