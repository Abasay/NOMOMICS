...
import { uploadBase64ImageToCloudinary, uploadBase64PdfToCloudinary } from '@src/util/cloudinary';
...
async function getComicsFileUrl(req: IReq, res: IRes) {
  ...
  const comicsUrl = await ComicsService.getComicsFileUrl(req.body.id as ObjectId, base64File);
  return res.status(HttpStatusCodes.OK).json({ success: true, data: { comicsUrl } });
}

async function uploadComic(req: IReq, res: IRes) {
  const [title, subTitle, description, category, author, synopsis, genre, location, coverImage, filesType] =
...
async function uploadComicPicsToCloudinary(req: IReq, res: IRes) {
  ...
  const imageUrl = (await uploadBase64ImageToCloudinary(base64File, title)) as string;
...
async function allComics(req: IReq, res: IRes) {
  ...
  const newComics = comics.map((comic) => {
    const episodes = comic.episodes.filter((episode) => episode.isApproved);
...
async function getComic(req: IReq, res: IRes) {
  ...
  if (comic) {
    if (episodeId) episodeIdExists = comic?.episodes.find((e) => Number(e.episodeNumber) === Number(episodeId));
    else episodeIdExists = comic?.episodes[0];

    const searchEpisode = comic?.episodes.find((e) => Number(e.episodeNumber) === Number(episodeId));
    console.log(typeof episodeId, episodeIdExists);

    if (searchEpisode === undefined && String(episodeId) !== 'null') {
...
async function getComments(req: IReq, res: IRes) {
  ...
  if (comic) {
    ...
    const episodeIdExists = comic?.episodes.find((e) => Number(e.episodeNumber) === Number(episodeId));
...
async function likeAndUnlikeAComment(req: IReq, res: IRes) {
  const [comicId, commentId, episodeId, userId, commentorId] = check.isStr(req.body, [
    'comicId',
    'commentId',
    'episodeId',
    'userId',
...
async function getNestedComments(req: IReq, res: IRes) {
  const [commentId, parentCommentId] = check.isStr(req.body, ['commentId']);
...
async function saveComic(req: IReq, res: IRes) {
  ...
  const save = await ComicsService.saveComic(comicId as unknown as ObjectId, userId as unknown as ObjectId);
...
async function followComic(req: IReq, res: IRes) {
  ...
  const follow = await ComicsService.followComic(comicId as unknown as ObjectId, userId as unknown as ObjectId);
...
async function getFollowers(req: IReq, res: IRes) {
  ...
  const followers = await followersModel.find({ comicId: comicId }).populate({
    path: 'userId',
...
async function viewComic(req: IReq, res: IRes) {
  const [comicId, userId, episodeId] = check.isStr(req.body, ['comicId', 'userId', 'episodeId']);
...
async function metrics(req: IReq, res: IRes) {
  ...
  if (user.role !== 'Creator') throw new RouteError(HttpStatusCodes.UNAUTHORIZED, 'User not authorized');

  const views = await viewModel.find({ authorId: user._id }).countDocuments();
  ...
  const saved = await saveModel.find({ authorId: user._id }).countDocuments();
  ...
  const episodes = await Comic.aggregate([
    ...
    },
    {
      $project: {
        ...
        episodeCoverImage: '$episodes.episodeCoverImage',
